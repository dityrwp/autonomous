...
from models.fusion import BEVFusion # Needs potential adaptation for single-stage fusion
...
class Trainer:
    ...
    def _create_model(self) -> Dict[str, nn.Module]:
        ...
        components['lidar_backbone'] = SECONDBackbone(
            ...
            max_voxels=lidar_config.get('max_voxels', 20000)
        )
        ...
        components['fusion'] = BEVFusion(
            ...
            chunk_size=fusion_config.get('chunk_size', 2048),
            use_reentrant=fusion_config.get('use_reentrant', False)
        )
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            if self.train_config.get('optimizer', {}).get('grad_clip'):
                ...
                torch.nn.utils.clip_grad_norm_(
                    [p for c in self.model_components.values() for p in c.parameters()],
                    grad_clip_value
                )
    ...
    def _visualize_predictions(self, images: torch.Tensor, predictions: torch.Tensor, targets: torch.Tensor, prefix: str = 'train'):
        ...
        # Save figure
        viz_path = self.viz_dir / f'{prefix}_predictions_epoch{self.epoch}.png'
...
# --- Custom Collate Function --- (Keep as is, seems reasonable)
def custom_collate_fn(batch):
    """Custom collate function to handle variable-sized point clouds."""
    images, lidar_points, bev_labels, calibs, sample_tokens, scene_tokens = [], [], [], [], [], []
    ...
    for item in batch:
        ...
        if 'calib' in item: calibs.append(item['calib'])
    ...
    batch_dict = {
        ...
        'lidar': lidar_points, # Keep list of tensors
        'bev_label': torch.stack(bev_labels)
    }
    # Add optional items
    if calibs: batch_dict['calib'] = calibs # Might need stacking/padding depending on use
...
def main():
    ...
    img_h, img_w = model_config.get('input_config', {}).get('image_size', [512, 896]) # Default if not set
    ...
    if args.augmentation_strength != 'none':
        ...
        # Define BEVAugmentor based on strength (example)
        if args.augmentation_strength == 'light':
             augmentor = BEVAugmentor(flip_prob=0.3, rotate_prob=0.3, rotate_range=(-5, 5))
        elif args.augmentation_strength == 'medium':
             augmentor = BEVAugmentor(flip_prob=0.5, rotate_prob=0.5, rotate_range=(-10, 10), scale_prob=0.3, translate_prob=0.3)
        elif args.augmentation_strength == 'strong':
             augmentor = BEVAugmentor(flip_prob=0.5, rotate_prob=0.7, rotate_range=(-15, 15), scale_prob=0.5, translate_prob=0.5, cutout_prob=0.3, mixup_prob=0.2)
