...
class Trainer:
    ...
    def _create_model(self) -> Dict[str, nn.Module]:
        ...
        bev_skip_channels = {
            'stage1': 32,  # Early features (highest resolution) - only skip channel needed now
        }
        ...
        components['fusion'] = BEVFusion(
            ...
            chunk_size=fusion_config.get('chunk_size', 1024),  # Increased from default 1024
        ...
        components['head'] = BEVSegmentationHead(
            ...
            skip_channels={'stage1': bev_skip_channels['stage1']},  # Only pass stage1 for skip connections
            ...
            dice_smooth=loss_config.get('dice_smooth', 100.0),  # Use increased dice_smooth value
    ...
    def train_epoch(self):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            # Check if debug.check_nan is enabled in train_config
            check_nan = self.train_config.get('debug', {}).get('check_nan', False)
            # Temporarily disable mixed precision if we're seeing NaN issues
            use_amp = self.scaler.is_enabled() and not check_nan

            with amp.autocast(enabled=use_amp):
                # 1. Camera Backbone & BEV Lifting
                image_feats_all_stages = self.camera_backbone(images)
                
                # Check for NaNs if enabled
                if check_nan and any(torch.isnan(f).any() for f in image_feats_all_stages.values()):
                    print("WARNING: NaN detected in image_feats_all_stages")
                    # Fix NaNs in features
                    for k in image_feats_all_stages.keys():
                        image_feats_all_stages[k] = torch.nan_to_num(image_feats_all_stages[k], nan=0.0)
                
                # Handle the dictionary output from the lifter
                lifter_output = self.image_lifter(
                    image_feats_all_stages,
                    calib
                )
                main_image_bev = lifter_output['bev_features']
                image_bev_skips = lifter_output['skip_features']
                
                # Check for NaNs if enabled
                if check_nan:
                    if torch.isnan(main_image_bev).any():
                        print("WARNING: NaN detected in main_image_bev")
                        main_image_bev = torch.nan_to_num(main_image_bev, nan=0.0)
                    for k, v in image_bev_skips.items():
                        if torch.isnan(v).any():
                            print(f"WARNING: NaN detected in image_bev_skips[{k}]")
                            image_bev_skips[k] = torch.nan_to_num(v, nan=0.0)
                ...
                # Check for NaNs if enabled
                if check_nan and torch.isnan(projected_lidar_bev).any():
                    print("WARNING: NaN detected in projected_lidar_bev")
                    projected_lidar_bev = torch.nan_to_num(projected_lidar_bev, nan=0.0)
                
                # 3. Fusion
                fused_features, _ = self.fusion(
                    projected_lidar_bev,
                    main_image_bev,
                    image_bev_skips # Pass the skips dictionary
                )
                
                # Check for NaNs if enabled
                if check_nan and torch.isnan(fused_features).any():
                    print("WARNING: NaN detected in fused_features")
                ...
                if torch.isnan(total_loss) or torch.isinf(total_loss):
                    print("WARNING: NaN or Inf detected in total_loss, using substitute loss")
            ...
            if (batch_idx + 1) % accumulation_steps == 0 or (batch_idx + 1) == total_batches:
                # Always apply gradient clipping with a proper value
                self.scaler.unscale_(self.optimizer) # Unscale gradients before clipping
                # Use either the config value or a default of 0.5
                grad_clip_value = self.train_config.get('optimizer', {}).get('grad_clip', 0.5)
                torch.nn.utils.clip_grad_norm_(
                    [p for c in self.model_components.values() for p in c.parameters() if p.grad is not None], 
                    grad_clip_value
                )
                
                # Check for inf/nan in gradients
                if any(p.grad is not None and (torch.isnan(p.grad).any() or torch.isinf(p.grad).any()) 
                       for c in self.model_components.values() for p in c.parameters()):
                    print("WARNING: NaN or Inf detected in gradients. Skipping optimizer step.")
                    # Skip optimizer step but update scaler
                    self.scaler.update()
                else:
                    # Normal step if no NaNs or Infs detected
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            # Check for NaNs if enabled
            if check_nan and any(torch.isnan(f).any() for f in image_feats_all_stages.values()):
                print("WARNING: NaN detected in image_feats_all_stages during validation")
                # Fix NaNs in features
                for k in image_feats_all_stages.keys():
                    image_feats_all_stages[k] = torch.nan_to_num(image_feats_all_stages[k], nan=0.0)
            ...
            if check_nan:
                if torch.isnan(main_image_bev).any():
                    print("WARNING: NaN detected in main_image_bev during validation")
                    main_image_bev = torch.nan_to_num(main_image_bev, nan=0.0)
                for k, v in image_bev_skips.items():
                    if torch.isnan(v).any():
                        print(f"WARNING: NaN detected in image_bev_skips[{k}] during validation")
                        image_bev_skips[k] = torch.nan_to_num(v, nan=0.0)
            ...
            # Check for NaNs if enabled
            if check_nan and torch.isnan(projected_lidar_bev).any():
                print("WARNING: NaN detected in projected_lidar_bev during validation")
                projected_lidar_bev = torch.nan_to_num(projected_lidar_bev, nan=0.0)
            ...
            # Check for NaNs if enabled
            if check_nan and torch.isnan(fused_features).any():
                print("WARNING: NaN detected in fused_features during validation")
                fused_features = torch.nan_to_num(fused_features, nan=0.0)
            ...
            if targets is not None:
                ...
                # Check for NaN loss
                if torch.isnan(total_loss) or torch.isinf(total_loss):
                    print("WARNING: NaN or Inf detected in total_loss during validation, using substitute")
            ...
            if torch.isnan(predictions).any():
                print("WARNING: NaN detected in predictions during validation")
...
def main():
    ...
    if args.debug:
        ...
        val_dataset = torch.utils.data.Subset(val_dataset, range(min(100, len(val_dataset))))
